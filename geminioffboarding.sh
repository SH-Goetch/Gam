...
export GAMCFGDIR="<span class="math-inline">HOME/GAMConfig"
...
retry_group_creation() {
  ...
  until $GAM_CMD create group $USER_EMAIL; do
    ...
    sleep <span class="math-inline">delay
delay\=</span>((delay * 2)) # Exponential backoff
  done
}
...
remove_all_aliases_after_delay() {
  ...
  sleep <span class="math-inline">delay
aliases\=</span>($GAM_CMD info user $USER_EMAIL | grep 'Alias:' | awk '{print $2}')
...
# 3. Wait 70 seconds before removing aliases
remove_all_aliases_after_delay
...
$GAM_CMD user $SUSPENDED_USER_EMAIL transfer calendars <span class="math-inline">MANAGER\_EMAIL
...
ARCHIVE\_FOLDER\_ID\=</span>($GAM_CMD user $MANAGER_EMAIL create drivefile targetfolder $ARCHIVE_LOCATION | grep 'id:' | awk '{print $2}')
